...
# 텍스트, oov.int ('1'), 트리, final.mdl이 재료임
...
sdata=$data/split$nj
splice_opts=`cat $srcdir/splice_opts 2>/dev/null` # 2: a file descriptor./ frame-splicing options.
...
# use graph = false, true 의미하는게 뭐지? true로 하면 이전 단계것을 그대로 쓴다는것. 새로 컴파일 (compile-train-graphs) 생략.
# 근데 이 graph라는게 뭐지?
if $use_graphs; then
